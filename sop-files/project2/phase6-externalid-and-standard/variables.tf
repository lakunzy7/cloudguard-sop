...
# ⚠️ CHANGE THIS: the value a caller must present to assume Project 2's
# vendor integration role.
#
# In production this is a shared secret, generated once and held by both
# sides — not a default committed to a repository, where it is readable
# by anyone with iam:GetRole on the role. It is a variable here so the
# demonstration can be reproduced, and the value below should be read as
# a placeholder rather than as an example of the practice. The
# walkthrough says so where the limitation matters.
variable "vendor_external_id" {
  description = "The ExternalId a caller must present to assume Project 2's vendor integration role."
  type        = string
  default     = "cloudguard-vendor-integration-2026"
  sensitive   = true
}
